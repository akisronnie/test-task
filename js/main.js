...
function chgcar() {
    ...
    switch (styleavto) {
        case 'МИНИМАЛЬНЫЙ':
            ...
            break;
        case 'СТАНДАРТ':
            { price += 10000; iconview = ['полкапот','оптика', 'зеркало', 'пбампер', 'ручки',  'полкрыло']; }
            break;
        case 'СТАНДАРТ+':
            { price += 11000; iconview = ['капот', 'оптика', 'зеркало', 'пбампер', 'ручки',  'полкрыло']; }
            break;
        case 'СТАНДАРТ++':
            { price += 12000; iconview = ['капот','оптика', 'зеркало', 'пбампер', 'ручки',  'крыло']; }
            break;
        case 'СТАНДАРТ+++':
            { price += 13000; iconview = ['капот','оптика', 'зеркало', 'пбампер', 'ручки',  'крыло', 'збампер']; }
            break;
        case 'ЦЕЛИКОМ':
            { price += 14000; iconview = ['капот','оптика', 'зеркало', 'пбампер', 'ручки',  'крыло', 'збампер', 'двери', 'пороги']; }
            break;
        case 'ОПТИКА':
